...
> 理论篇讲了权限系统的四层防御和权限模式矩阵，这篇带你走读 Python 版 MewCode 的真实代码，看看五个文件、不到 300 行是怎么把这套防御体系实现出来的。
...
_DANGEROUS_PATTERNS: list[
    ...
    (re.compile(
        r"rms+-[a-z]*r[a-z]*f[a-z]*s+/s*$"),
     "递归强制删除根目录"),
    (re.compile(r"mkfs."), "格式化磁盘"),
    (re.compile(r"dds+if=.*of=/dev/"),
     "直接写磁盘设备"),
    (re.compile(r"chmods+-Rs+777s+/"),
     "递归修改根目录权限"),
    (re.compile(r":(){s*:|:&s*};:"),
     "fork bomb"),
    (re.compile(r"curls+.*|s*(ba)?sh"),
     "管道执行远程脚本"),
    (re.compile(r"wgets+.*|s*(ba)?sh"),
     "管道执行远程脚本"),
...
看几条有代表性的： `rms+-[a-z]*r[a-z]*f[a-z]*s+/s*$` 匹配 `rm -rf /` ，但不是死板的字面匹配。 `[a-z]*r[a-z]*f[a-z]*` 能匹配 `rf` 、 `rif` 、 `rfv` 等各种组合，因为 rm 的参数可以混合在一起写。 `s+/s*$` 要求目标是根目录且在命令末尾。
...
用 `fnmatch` 而不是正则，是因为权限规则面向的是普通用户， `Bash(git *)` 比 `Bash(gits+.*)` 直观得多。
