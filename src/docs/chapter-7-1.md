...
出在 **工具的供给和 Agent 的核心是耦合的** 。Agent 想用新工具，必须由 Agent 的开发者来写代码、编译、发布。第三方开发者没有办法独立地给 MewCode 添加工具。

![](images/chapter-7-1/img-1.png)
...
![](images/chapter-7-1/img-3.png)

MCP 标准协议连接 Agent 与 Server
...
![](images/chapter-7-1/img-4.png)

Host Client Server 角色关系
...
![](images/chapter-7-1/img-5.png)

MCP Data Layer 和 Transport Layer
...
那 WebSocket 呢？规范允许实现自定义 transport，但 **WebSocket 不是当前标准传输的一部分** 。所以不要把它和 `stdio` 、 `Streamable HTTP` 并列成“官方标准的三四种传输”。
...
![](images/chapter-7-1/img-8.png)

stdio 与传统 RPC 对比

你想想传统的 RPC 通信是怎么做的。服务端得监听一个端口，比如 [localhost:8080](http://localhost:8080/) ，客户端连接过去。这带来一堆麻烦：端口可能被占用，得处理冲突。防火墙可能阻拦本地连接，得配置规则。如果同时跑多个 MCP Server，每个都要占一个端口，还得管理端口分配。进程挂了端口可能不会立即释放，下次启动会报「address already in use」。
...
![](images/chapter-7-1/img-9.png)

stdin stdout stderr 通道边界
...
![](images/chapter-7-1/img-15.png)

ToolRegistry 统一管理 MCP 工具
