...
> 理论篇讲了两层上下文管理的设计，这篇带你走读 Python 版 MewCode 的真实代码。一个文件 670 行，用三趟扫描加一次 LLM 摘要再叠一段恢复块，把「对话太长了怎么办」以及「压缩之后模型怎么不失忆」一起解决得很干净。
...
```python
SUMMARY_PROMPT = """你是一个对话摘要助手。你只能输出纯文本，不能调用任何工具。
请对下面的对话生成一份结构化摘要。
...
`_truncate_by_tokens` 按 `len(s) / 3.5` 折算 byte 上限，超额时切尾追加 `… (内容已截断)` 标记。这个标记是给模型的提示，让它知道这里不是完整内容，需要的话要重读。
...
`+ 8` 是给 `###` 这种 markdown 开销留的小余量，估算用，不需要精确。算到超预算就 break，保证总长可控。
...
`build_compact_messages` 多了一个 `attachment` 参数，把它用 `---` 拼到 `[摘要]
{summary}` 之后：
