...
![](images/chapter-12-1/img-2.png)

开发者反复手动做同样操作的人肉 CI 场景
...
![](images/chapter-12-1/img-3.png)

手动重复操作与 Hook 自动化的对比图

## Hook 的三要素

一个 Hook 由三部分组成：事件、条件、动作。用人话说就是「什么时候」「什么情况下」「做什么」。

![](images/chapter-12-1/img-4.png)

Hook 的事件、条件、动作三要素示意图
...
![](images/chapter-12-1/img-5.png)

Agent 生命周期事件总览时间线

先是 **会话级事件** 。session\_start 在新会话开始时触发，session\_end 在会话结束时触发。一个会话从开到关，这两个事件各自只发生一次。再细一层是 **轮次级事件** 。turn\_start 在用户发送新消息时触发，标志着一轮对话的开始；turn\_end 在 Agent 完成回复时触发，标志着一轮对话的结束。一个会话里通常有很多轮对话，所以这两个事件会反复出现，颗粒度比会话级细很多。
...
![](images/chapter-12-1/img-6.png)

pre\_tool\_use 是唯一可拦截事件的强调图

## pre\_tool\_use：唯一能说「不」的事件
...
![](images/chapter-12-1/img-8.png)

ToolRejectedError 触发的反馈循环图
...
![](images/chapter-12-1/img-10.png)
...
![](images/chapter-12-1/img-11.png)
...
![](images/chapter-12-1/img-12.png)

command 执行器正常完成与超时对比
...
![](images/chapter-12-1/img-13.png)

System Prompt 与 Hook Prompt 注入对比
...
![](images/chapter-12-1/img-14.png)

agent 执行器让 AI 监督 AI 的示意图

不过 agent 执行器要真正跑起来，得依赖下一章（第 13 章 SubAgent）讲的子 Agent 运行时。本章我们把执行器的接口和调用骨架搭好就行：Hook 配置能加载、能校验通过，至于子 Agent 怎么真的被启动、上下文怎么传递，留到 第 13 章再展开。各语言版本在这里的过渡策略略有不同，Go 版本通过 `AgentRunner` 回调由上层注入（保持 Hook 模块零外部依赖），Python 版本则是更彻底的占位 stub，效果上都是「本章先接口，下一章再对接真实运行时」。
...
![](images/chapter-12-1/img-15.png)

once true 只在第一次触发的时间线
...
![](images/chapter-12-1/img-16.png)
...
![](images/chapter-12-1/img-17.png)
...
![](images/chapter-12-1/img-18.png)
...
![](images/chapter-12-1/img-19.png)

Agent Loop 中 Hook 插入点总览
...
![](images/chapter-12-1/img-20.png)
