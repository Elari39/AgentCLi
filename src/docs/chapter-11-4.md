...
执行器持有 Agent 和 LLM Client 的引用。 `execute_inline` 和 `execute_fork` 两个方法分别处理两种执行模式。inline 直接调 Agent 的方法注入 prompt，fork 创建一个独立的 Agent 实例，逻辑很显式。
...
| 上下文传递 | 三种模式：`none`（不传）、`recent`（最近 5 条）、`full`（每条截断 200 字符的摘要） |
